...
function loadStatsPage(){
	...
	for(row_counter = 2; row_counter < table.rows.length; row_counter++)
	{
		if(parseInt(table.rows[row_counter].cells[2].innerHTML) > parseInt(table.rows[row_counter].cells[3].innerHTML)){
			table.rows[row_counter].cells[4].innerHTML = "Colorado Boudler";
			winCount++;
		}
		else{
...
function loadPlayersPage(){
	...
	for(let j = 0; j<numberofPlayers; j++){
		dropdownmenu.innerHTML += '<a class = "dropdown-item" href = "#" onclick = "switchPlayers(' + j + ')">' + players[j].name + '</a>';/*shows number in drop down menu */
	}
}

function switchPlayers(playerNum){
	document.getElementById("selectPlayerButton").innerHTML = players[playerNum].name; /*self explanitory, using getelementbyid and innerHTML to find everything
	...
	var numberofGames = parseInt(players[playerNum].games_played);

	document.getElementById("avg_p_yards").innerHTML = Math.floor(parseInt(players[playerNum].pass_yards)/numberofGames);
	document.getElementById("avg_r_yards").innerHTML = Math.floor(parseInt(players[playerNum].rushing_yards)/numberofGames);
	document.getElementById("avg_rec_yards").innerHTML = Math.floor(parseInt(players[playerNum].receiving_yards)/numberofGames);
}
